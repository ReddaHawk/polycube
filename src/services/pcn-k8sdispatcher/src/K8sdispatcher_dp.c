...
static int handle_rx(struct CTXTYPE *ctx, struct pkt_metadata *md) {
  ...
  case htons(ETH_P_ARP):
     ...
     */
    pcn_log(ctx, LOG_TRACE, "Received ARP packet. Letting it go through from %d",md->in_port);
    return RX_OK;
    break;
  default:
    pcn_log(ctx, LOG_TRACE, "Unknown eth proto: %d, send to stack",
            bpf_ntohs(eth->proto));
    return RX_OK;
  }
  ...
  pcn_log(ctx, LOG_INFO, "Processing IP packet: src %I, dst: %I", ip->saddr,
          ip->daddr);
// check if ip dest is for this host
  if (ip->daddr != EXTERNAL_IP && md->in_port == FRONTEND_PORT) {
  ...
  if (md->in_port == BACKEND_PORT) {
    ...
    if (value != NULL) {
      pcn_log(ctx, LOG_INFO, "Egress rule table: hit");
    ...
    pcn_log(ctx, LOG_INFO, "Egress rule table: miss");
     */
    // Packet is inside -> outside, check SNAT/MASQUERADE rule table
    newIp = EXTERNAL_IP;
    newPort = get_free_port();
    ...
    pcn_log(ctx, LOG_INFO, "Ingress rule table: miss");
    //todo check
    return RX_OK;
  }
  ...
  // Modify packet
  uint32_t old_ip =
      (rule_type == NAT_SRC ) ? srcIp : dstIp;
  uint32_t old_port =
      (rule_type == NAT_SRC ) ? srcPort : dstPort;
...
static __always_inline int handle_rx(struct CTXTYPE *ctx,
  ...
  if(BACKEND_PORT==FRONTEND_PORT){
    pcn_log(ctx, LOG_TRACE,"Not ready");
    return RX_OK;
  }
  if(md->in_port == BACKEND_PORT){
    pcn_log(ctx, LOG_TRACE, "frontend: %d backend: %d in %d",FRONTEND_PORT,BACKEND_PORT, md->in_port);
  ...
  case IPPROTO_ICMP: {
    ...
    if ((void *)icmp + sizeof(*icmp) > data_end)
      goto DROP;
    pcn_log(ctx, LOG_INFO, "Packet is ICMP: type %d, id %d from port %d", icmp->type,
            icmp->un.echo.id,md->in_port);
    // Consider the ICMP ID as a "port" number for easier handling
    srcPort = icmp->un.echo.id;
    dstPort = icmp->un.echo.id;
    break;
  ...
  if (md->in_port == FRONTEND_PORT) {
    if(proto == IPPROTO_ICMP)
      pcn_log(ctx, LOG_INFO, "x----ICMP packet: src %I, dst %I from port %d", srcIp, dstIp, md->in_port);
    // Packet is outside -> inside, check ingress session table
    value = ingress_session_table.lookup(&key);
    if (value != NULL) {
      // Session table hit
      pcn_log(ctx, LOG_INFO, "Ingress session table: hit");

      newIp = value->new_ip;
      newPort = value->new_port;
      pcn_log(ctx, LOG_INFO, "Ingress session table: hit with newport = %P",newPort);
      // rule_type = NAT_DST;
    ...
    if (value != NULL) {
      // Session table hit
      pcn_log(ctx, LOG_INFO, "Egress session table: hit");
    ...
    pcn_log(ctx, LOG_INFO, "Egress session table: miss");
  }
  // Session table miss
  if (md->in_port == BACKEND_PORT) {
    // Packet is inside -> outside, check SNAT/MASQUERADE rule table
...
apply_nat:;
  if (update_session_table == 1) {
    pcn_log(ctx, LOG_INFO, "Update session table");
    // No session table exist for the packet, but a rule matched
    // -> Update the session tables in both directions
  ...
  case IPPROTO_ICMP: {
    ...
    if ((void *)icmp + sizeof(*icmp) > data_end)
      goto DROP;
    pcn_log(ctx, LOG_INFO, "newip %I",new_ip);
    if(md->in_port == FRONTEND_PORT){
      ip->daddr = new_ip;
      pcn_log(ctx, LOG_INFO, "esco con da frontend con dest newip %I and newport %P",
             new_ip,(__be16)new_port);
    }
    else{
      ip->saddr=new_ip;
      pcn_log(ctx, LOG_INFO, "esco da backend con source newip %I and newport %P",
              new_ip,(__be16)new_port);
    }
    icmp->un.echo.id = (__be16)new_port;
    pcn_log(ctx, LOG_INFO, "Natted ICMP packet: source, %I:%P -> %I:%P",
            old_ip, old_port, new_ip, new_port);
    //ip->daddr = new_ip;
    //icmp->un.echo.id = (__be16)new_port;
    //pcn_log(ctx, LOG_INFO, "Natted ICMP packet: destination, %I:%P -> %I:%P",
    //        old_ip, old_port, new_ip, new_port);
...
proceed:;
  if(md->in_port==FRONTEND_PORT){
    //pcn_log(ctx, LOG_INFO, "Packet from frontend to backend src: %I, dst: %I, macSrc: %M, macDst:%m", ip->saddr, ip->daddr,eth->src,eth->dst);
    pcn_log(ctx, LOG_INFO, "Packed from frontend to backend");
    return pcn_pkt_redirect(ctx, md, BACKEND_PORT);
  } else
    pcn_log(ctx, LOG_INFO, "Packed from backend to frontend");
  //pcn_log(ctx, LOG_INFO, "Packet from backend to frontend src: %I, dst: %I, macSrc: %M, macDst:%m", ip->saddr, ip->daddr,eth->src,eth->dst);
  return pcn_pkt_redirect(ctx, md, FRONTEND_PORT);

DROP:;
  //pcn_log(ctx, LOG_INFO, "Drop packet");
  return RX_OK;
}
*/
